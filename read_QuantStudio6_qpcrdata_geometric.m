function [package] = read_QuantStudio6_qpcrdata_geometric(filename,ctrl_Tar,ctrl_Samp)
...
if exist([filename2save '.mat'])==2
   ...
   [WellPos_s0,ti0]=sort(WellPos);
    SampName_s0=SampName(ti0);
    TarName_s0=TarName(ti0);
    Ct_s0=Ct_raw(ti0);
    Omit_s0=Omit(ti0);  
    
    [SampName_s1,ti]=sort(SampName_s0);
    TarName_s1=TarName_s0(ti);
    WellPos_s1=WellPos_s0(ti);
    Ct_s1=Ct_s0(ti);
    Omit_s1=Omit_s0(ti);;
...
%changes!!!
dCtSD_samp_by_tar=stdNaN3(dCt_samp_by_tar);
...
mean_dFold_samp_by_tar=meanNaN3(dFold_samp_by_tar,'geo');
...
if length(ctrl_Samp_ind)==1
    ...
    %sd_ddFold_samp_by_tar=zeros(size_mat(1)-1,size_mat(2)-1,2);
else
    %mean_ddFold_samp_by_tar=zeros(size_mat(1),size_mat(2));
    %sd_ddFold_samp_by_tar=zeros(size_mat(1)-1,size_mat(2),2);
end
for m=1:size(dFold_samp_by_tar,1)
    mean_ddFold_samp_by_tar(m,:)=mean_dFold_samp_by_tar(m,:)./mean_dFold_samp_by_tar(ctrl_Samp_ind,:);
    %sd_ddFold_samp_by_tar(m,:)=sd_dFold_samp_by_tar(m,:)./mean(mean_dFold_samp_by_tar(ctrl_Samp_ind,:),1);
end
%changes!!!
sd_ddFold_samp_by_tar(:,:,1)=mean_ddFold_samp_by_tar-(2.^(log2(mean_ddFold_samp_by_tar)-dCtSD_samp_by_tar));
sd_ddFold_samp_by_tar(:,:,2)=(2.^(log2(mean_ddFold_samp_by_tar)+dCtSD_samp_by_tar))-mean_ddFold_samp_by_tar;
