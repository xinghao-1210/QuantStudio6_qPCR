function [package] = read_QuantStudio6_Taqmandata(filename,ctrl_Tar,ctrl_Samp)
...
len_tn=length(filename);
if strcmpi(filename((len_tn-3):(len_tn)),'.xls')
    filename2save=[filename(1:(length(filename)-4))];
elseif strcmpi(filename((len_tn-4):(len_tn)),'.xlsx')
...
if exist([filename2save '.mat'])==2
    ...
    [dat,head,raw]=xlsread(filename,'Results','A15:Z700');
    savelater=true;
    rn=check_range_row(raw,2);
    ...
    Ct_raw=-1*Ct_raw;
    for k=1:max(size(rn))
        if isa(raw{rn(k),13},'double') && ~isnan(raw{rn(k),13})
            Ct_raw(k)=raw{rn(k),13};
        else
            if NaN_is_num
                Ct_raw(k)=NaNnum;
            else
...
dCt_samp_by_tar=NaN(size_mat(1)-1,size_mat(2)-length(ctrl_Tar_ind),size_mat(3));
...
function rn = check_range_row(c,coln)
k=1;
while true %for k=2:size(c,1)
   if isa(c{k,coln},'double')
       if c{k,coln}==1
           b=k;
           break
       end
